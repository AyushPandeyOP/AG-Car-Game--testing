// script.js - FIXED + MORNING WEATHER 🌄 + PALM TREES 🌴 + BETTER SPEED + CAR ON ROAD
let scene, camera, renderer;
let car, wheels = [], flame, spoiler;
let roadSegments = [];
let traffic = [];
let sideObjects = []; // now palm trees
let keys = {};
...
let paused = false;
let sensitivity = 1.2; // increased for better left/right feel
...
const MAX_SPEED = 260;          // balanced speed (was 280)
const ROAD_SEGMENTS_COUNT = 10;

function initThree() {
    const container = document.getElementById('container');
    scene = new THREE.Scene();
    scene.fog = new THREE.Fog(0xaaccff, 80, 420); // morning fog
    ...
    scene.add(hemi);
    const sun = new THREE.DirectionalLight(0xffeeaa, 1.7); // warm morning sun
    sun.position.set(80, 130, -40);
    sun.castShadow = true;
    sun.shadow.mapSize.width = 1024;   // optimized for graphics fix
    ...
    createCar();
    createRoad();
    createEnvironment();   // now palm trees 🌴
...
function createCar() {
    ...
    scene.add(car);
    car.position.set(0, 0.1, 0); // slightly raised
}
...
function createEnvironment() {
    ...
    for (let i = 0; i < 22; i++) {  // more palms
        ...
        // Fronds (multiple for realistic palm look)
        for (let k = 0; k < 7; k++) {
    ...
    // Road edge barriers (to keep car on road)
...
function updatePhysics(delta) {
    let accel = 0;
    if (keys['w'] || keys['arrowup'] || mobile.accel) accel = 2.4;   // stronger acceleration fix
    if (keys['s'] || keys['arrowdown'] || mobile.brake) accel = -2.0;

    carSpeed += accel * 85 * delta;
    carSpeed *= 0.93; // smoother drag
    carSpeed = Math.max(15, Math.min(MAX_SPEED, carSpeed));

    isBoosting = (keys['shift'] || mobile.nitro) && nitroLevel > 2;
    if (isBoosting) {
        carSpeed = Math.min(MAX_SPEED + 75, carSpeed + 140 * delta);
    ...
    // STRONGER ROAD CLAMP - Car stays on road 🛣️
    const edge = ROAD_WIDTH / 2 - 3;
    if (car.position.x < -edge) car.position.x = -edge + 0.3; // bounce back
    ...
    car.position.y = Math.sin(Date.now() * 0.02) * (carSpeed / 220) * 0.2 + 0.1; // fixed bottom floating
...
// Rest of the functions (startGame, pause, settings, etc.) remain the same
function startGame() {
    document.getElementById('start-screen').classList.remove('active');
    document.getElementById('hud').classList.remove('hidden');
    if (!scene) initThree();
    carSpeed = 58; nitroLevel = 100; distance = 0; car.position.set(0, 0.1, 0); car.rotation.set(0,0,0);
...
window.onload = () => console.log('%c🚗 VELOCITY RUSH - Morning 🌄 + Palm Trees 🌴 + Fixed Controls (by Ayush Pandey JI)', 'color:#ffee88; font-size:15px');